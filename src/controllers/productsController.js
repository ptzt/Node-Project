...
const updateProduct = async (req, res) => {
    try {
        const { id_p } = req.params
        const connection = await getConnection()
        const result = await connection.query("UPDATE products SET ? WHERE id_p = ?", [req.body, id_p])
...
const deleteProduct = async (req, res) => {
    try {
        const { id_p } = req.params
        const connection = await getConnection()
        await connection.query("DELETE FROM products WHERE id_p = ?", id_p)
        await connection.query("DELETE FROM product_order WHERE id_product = ?", id_p)
